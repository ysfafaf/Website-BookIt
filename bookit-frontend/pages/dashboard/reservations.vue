...
const handleCancel = async (id: string) => {
  ...
  try {
    ...
    error.value = err.message || 'Failed to cancel.'
  }
}
  ...
  <div>
    ...
    <div class="page-header-row">
      ...
      <NuxtLink to="/dashboard/reserve"
        style="display:inline-flex;align-items:center;gap:6px;background:#D97706;color:#fff;padding:10px 20px;border-radius:9px;font-weight:600;font-size:14px;text-decoration:none;white-space:nowrap;">
        + Book a Table
      </NuxtLink>
    </div>
      ...
      >
        <div class="reservation-card-inner">
          ...
          <div style="flex:1;min-width:0;">
            ...
            <div style="font-size:13px;color:#78716C;margin-bottom:2px;">
              Table T{{ String(r.table.table_number).padStart(2,'0') }}
            </div>
            <div v-if="r.description"
              style="font-size:12px;color:#A8A29E;font-style:italic;margin-top:2px;">
              "{{ r.description }}"
            </div>
          </div>
          ...
          <div style="flex-shrink:0;">
            <button
              ...
              style="padding:7px 14px;background:#FEE2E2;color:#991B1B;border:none;border-radius:9px;font-size:13px;font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif;white-space:nowrap;"
            >
              Cancel
            </button>
          </div>
